...
def classNames = ["Tuft"]   // names of classes of interest (simply add more values to list to add more classes)
double downsample = 4  // which downsampling level to use -> how much to downsample the patches
double glassThreshold = 10  // which threshold to use for tissue detection (lower value => more tissue included in mask)
